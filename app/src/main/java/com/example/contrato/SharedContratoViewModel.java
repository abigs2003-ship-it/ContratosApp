...
public class SharedContratoViewModel extends ViewModel {
    ...
    public void fetchContratoPorId(long idContrato) {
        Log.d("HISTORIAL", "Buscando contrato en BD. ID = " + idContrato);
        new Thread(() -> {
            try {
        ...
        new Thread(() -> {
            try {
                ...
                if (vc != null) {
                    ...
                    vc.idUsuarioModificacion = idUsuario;
                    Log.d("IDIOMA_DEBUG", "actualizaContratoBaseDatos: model.getIdioma()=" + model.getIdioma() + " -> mapeado=" + mapeaIdiomaBD(model.getIdioma()));
                    vc.idioma = orEmpty(mapeaIdiomaBD(model.getIdioma()));
                    vc.estatus = "A"; // se mantiene activo; ya no existe el estado "M"
                ...
                vigNuevo.pais         = orEmpty(truncate(model.getPais(), 50));
                vigNuevo.nacionalidad = orEmpty(truncate(model.getNacionalidad(), 50));
                vigNuevo.tipoDir      = orEmpty(model.getTipoDir());


                vigNuevo.calle = vigNuevo.noExt = vigNuevo.noInt = "";
                vigNuevo.poBox = vigNuevo.box = vigNuevo.cmr = vigNuevo.apo = "";
                vigNuevo.colonia = vigNuevo.delegacion = vigNuevo.ciudad = vigNuevo.estado = vigNuevo.cp = "";
                vigNuevo.linea1 = vigNuevo.linea2 = vigNuevo.linea3 = vigNuevo.linea4 = vigNuevo.linea5 = "";

                if ("México".equalsIgnoreCase(model.getPais())) {
                    vigNuevo.calle      = orEmpty(truncate(model.getMexCalle(), 150));
                    vigNuevo.noExt      = orEmpty(truncate(model.getMexNumExt(), 10));
                    vigNuevo.noInt      = orEmpty(truncate(model.getMexNumInt(), 10));
                    vigNuevo.colonia    = orEmpty(truncate(model.getMexColonia(), 50));
                    vigNuevo.delegacion = orEmpty(truncate(model.getDelegacion(), 50));
                    vigNuevo.ciudad     = orEmpty(truncate(model.getMexCiudad(), 50));
                    vigNuevo.estado     = orEmpty(truncate(model.getMexEstado(), 50));
                    vigNuevo.cp         = orEmpty(truncate(model.getMexCP(), 15));
                    ...
                    vigNuevo.calle   = orEmpty(truncate(model.getUsaCalle(), 150));
                    vigNuevo.ciudad  = orEmpty(truncate(model.getUsaCity(), 50));
                    vigNuevo.estado  = orEmpty(truncate(model.getUsaState(), 50));
                    vigNuevo.cp      = orEmpty(truncate(model.getUsaZip(), 15));
                    vigNuevo.poBox   = orEmpty(truncate(model.getPoBox(), 10));
                    vigNuevo.box     = orEmpty(truncate(model.getBox(), 10));
                    vigNuevo.cmr     = orEmpty(truncate(model.getCmr(), 10));
                    vigNuevo.apo     = orEmpty(truncate(model.getApo(), 10));
                } else if ("Canadá".equalsIgnoreCase(model.getPais())) {
                    vigNuevo.calle  = orEmpty(truncate(model.getCanCalle(), 150));
                    vigNuevo.ciudad = orEmpty(truncate(model.getCanCity(), 50));
                    vigNuevo.estado = orEmpty(truncate(model.getCanProvince(), 50));
                    vigNuevo.cp     = orEmpty(truncate(model.getCanPostalCode(), 15));
                } else {
                    vigNuevo.linea1 = orEmpty(truncate(model.getLinea1(), 150));
                    vigNuevo.linea2 = orEmpty(truncate(model.getLinea2(), 150));
                    vigNuevo.linea3 = orEmpty(truncate(model.getLinea3(), 150));
                    vigNuevo.linea4 = orEmpty(truncate(model.getLinea4(), 150));
                    vigNuevo.linea5 = orEmpty(truncate(model.getLinea5(), 150));
                    vigNuevo.pais   = orEmpty(truncate(model.getPaisOtro(), 50));
                }
                ...
                for (ContratoModelo.InfoTelefono p : model.getTelefonos()) {
                    ...
                    if      (p.etiqueta.contains("Casa 1"))    { vigNuevo.ladaCasa1      = orEmpty(truncate(cleanLada, 5)); vigNuevo.telefonoCasa1      = orEmpty(truncate(cleanNum, 15)); vigNuevo.whatsAppCasa1      = p.isWhatsApp; }
                    else if (p.etiqueta.contains("Casa 2"))    { vigNuevo.ladaCasa2      = orEmpty(truncate(cleanLada, 5)); vigNuevo.telefonoCasa2      = orEmpty(truncate(cleanNum, 15)); vigNuevo.whatsAppCasa2      = p.isWhatsApp; }
                    else if (p.etiqueta.contains("Celular 1")) { vigNuevo.ladaCelular1   = orEmpty(truncate(cleanLada, 5)); vigNuevo.telefonoCelular1   = orEmpty(truncate(cleanNum, 15)); vigNuevo.whatsAppCelular1   = p.isWhatsApp; }
                    else if (p.etiqueta.contains("Celular 2")) { vigNuevo.ladaCelular2   = orEmpty(truncate(cleanLada, 5)); vigNuevo.telefonoCelular2   = orEmpty(truncate(cleanNum, 15)); vigNuevo.whatsAppCelular2   = p.isWhatsApp; }
                    else if (p.etiqueta.contains("Celular 3")) { vigNuevo.ladaCelular3   = orEmpty(truncate(cleanLada, 5)); vigNuevo.telefonoCelular3  = orEmpty(truncate(cleanNum, 15)); vigNuevo.whatsAppCelular3   = p.isWhatsApp; }
                    else if (p.etiqueta.contains("Oficina 1")) { vigNuevo.ladaOficina1   = orEmpty(truncate(cleanLada, 5)); vigNuevo.telefonoOficina1   = orEmpty(truncate(cleanNum, 15)); vigNuevo.whatsAppOficina1   = p.isWhatsApp; }
                    else if (p.etiqueta.contains("Oficina 2")) { vigNuevo.ladaOficina2   = orEmpty(truncate(cleanLada, 5)); vigNuevo.telefonoOficina2   = orEmpty(truncate(cleanNum, 15)); vigNuevo.whatsAppOficina2   = p.isWhatsApp; }
                    else if (p.etiqueta.contains("Mensajes"))  { vigNuevo.ladaMensajes   = orEmpty(truncate(cleanLada, 5)); vigNuevo.telefonoMensajes   = orEmpty(truncate(cleanNum, 15)); vigNuevo.whatsAppMensajes   = p.isWhatsApp; }
                    if (p.esPrincipal) vigNuevo.telefonoDefault = orEmpty(truncate(p.etiqueta, 20));
                }

                vigNuevo.email1 = orEmpty((model.getEmails().size() > 0) ? truncate(model.getEmails().get(0), 60) : "");
                vigNuevo.email2 = orEmpty((model.getEmails().size() > 1) ? truncate(model.getEmails().get(1), 60) : "");
                vigNuevo.email3 = orEmpty((model.getEmails().size() > 2) ? truncate(model.getEmails().get(2), 60) : "");
                vigNuevo.email4 = orEmpty((model.getEmails().size() > 3) ? truncate(model.getEmails().get(3), 60) : "");
                ...
                Map<Long, VentasTitulares> archivosExistentes = new HashMap<>();
                for (VentasTitulares vt : titularesActuales) {
                    archivosExistentes.put(vt.idTitular, vt);
                }

                if (titularesCambiaron) {
                    titularesRepo.desactivarPorTipo(idContrato, "Titular", idUsuario);
                    guardaTitulares(model.getTitulares(), idContrato, "Titular", originalIdUsuarioAlta, archivosExistentes);
                }
                if (beneficiariosCambiaron) {
                    titularesRepo.desactivarPorTipo(idContrato, "Beneficiario", idUsuario);
                    guardaTitulares(model.getBeneficiarios(), idContrato, "Beneficiario", originalIdUsuarioAlta, archivosExistentes);
                }
                // ── Inventario ───────────────────────────────────────────────────
                ...
                viNuevo.unidad                  = orEmpty(model.getUnidad());
                viNuevo.temporada               = orEmpty(model.getTemporada());
                viNuevo.tipoVenta               = orEmpty(model.getTipoVenta());
                viNuevo.tipoOcupacion           = orEmpty(model.getTipoOcupacion());
                viNuevo.aniosComprados          = parseInt(model.getNoAnios());
                viNuevo.primerAnioUso           = parseLong(model.getAnioUso());
                viNuevo.monedaVenta             = orEmpty(model.getMoneda());
                ...
                viNuevo.tipoPago                = orEmpty(model.getTipoPago());
                ...
                viNuevo.tipoPagoDiferido = orEmpty(model.getTipoPagoEnganche());
                ...
                viNuevo.comentariosRegalos      = orEmpty(model.getComentarios());
                ...
                if (descuentosRepo.huboCambios(descuentosActuales, model.getDescuentosDetalle())) {
                    ...
                    for (ContratoModelo.DescuentoDetalle dd : model.getDescuentosDetalle()) {
                        ...
                        vd.descripcion    = orEmpty(dd.descripcion);
                ...
                vfNuevo.tipoPeriodo     = orEmpty(model.getTipoPeriodo());
                ...
                if (!model.isNoRedesSociales()) {
                    for (ContratoModelo.CuentaRed sa : model.getRedesSociales()) {
                        if      ("Instagram".equalsIgnoreCase(sa.red)) vrsNuevo.usuarioInstagram = orEmpty(sa.usuario);
                        else if ("Facebook".equalsIgnoreCase(sa.red))  vrsNuevo.usuarioFacebook  = orEmpty(sa.usuario);
                        else if ("Twitter".equalsIgnoreCase(sa.red) || "X".equalsIgnoreCase(sa.red)) vrsNuevo.usuarioTwitter = orEmpty(sa.usuario);
                    }
                }
    ...
    private void guardaTitulares(List<ContratoModelo.Persona> personas, long idContrato, String tipo,
                                 long idUsuarioAlta, Map<Long, VentasTitulares> archivosExistentes) throws SQLException {
        ...
        for (ContratoModelo.Persona p : personas) {
            ...
            // Busca el registro anterior por idTitular (estable), no por nombre
            VentasTitulares anterior = (p.idTitularBD > 0 && archivosExistentes != null)
                    ? archivosExistentes.get(p.idTitularBD)
                    : null;

            // Por default conserva lo que ya había en BD
            vt.archivoFirma      = (anterior != null) ? anterior.archivoFirma      : null;
            vt.archivoINEFrente  = (anterior != null) ? anterior.archivoINEFrente  : null;
            vt.archivoINEReverso = (anterior != null) ? anterior.archivoINEReverso : null;
            vt.archivoPasaporte  = (anterior != null) ? anterior.archivoPasaporte  : null;

            String base = (p.nombre + (p.paterno != null ? p.paterno : "") + (p.materno != null ? p.materno : ""))
                    .trim().toUpperCase().replaceAll(" ", "");

            boolean firmaNueva      = p.imagenFirmaBase64 != null && !p.imagenFirmaBase64.isEmpty();
            boolean ineFrenteNueva  = p.imagenINEFrente    != null && !p.imagenINEFrente.isEmpty();
            boolean ineReversoNueva = p.imagenINEReverso   != null && !p.imagenINEReverso.isEmpty();
            boolean pasaporteNueva  = p.imagenPasaporte    != null && !p.imagenPasaporte.isEmpty();

            // Solo se genera nombre nuevo si de verdad hay imagen nueva capturada
            if (firmaNueva)      vt.archivoFirma      = "idApp-" + idContrato + "-" + base + ".jpg";
            if (ineFrenteNueva)  vt.archivoINEFrente   = "idApp-" + idContrato + "-" + base + "-INE-FRENTE.jpg";
            if (ineReversoNueva) vt.archivoINEReverso  = "idApp-" + idContrato + "-" + base + "-INE-REVERSO.jpg";
            if (pasaporteNueva)  vt.archivoPasaporte   = "idApp-" + idContrato + "-" + base + "-PASAPORTE.jpg";

            titularesRepo.insert(vt);
            p.idTitularBD = vt.idTitular; // se actualiza para la próxima edición
    ...
    private void subirImagenAlBackend(String endpoint, long idTitular,
        ...
        FotoINERequest ineRequest = new FotoINERequest(
                (int) idTitular, nombreCompleto, idContrato, base64, lado, "jpg"
        );
        FotoPasaporteRequest pasRequest = new FotoPasaporteRequest(
                (int) idTitular, nombreCompleto, idContrato, base64, "jpg"
        );
        ...
        call.enqueue(new Callback<ApiResponse>() {
            ...
            public void onResponse(Call<ApiResponse> call, Response<ApiResponse> response) {
                if (response.isSuccessful() && response.body() != null && response.body().isExito()) {
                    String ruta = response.body().getRutaFirma();
                    Log.d("IMG_UPLOAD", "[" + endpoint + "] Subida OK: " + ruta);
                    String tipoArchivo = "ine/frente".equals(endpoint) ? "INE_FRENTE"
                            : "ine/reverso".equals(endpoint) ? "INE_REVERSO" : "PASAPORTE";
                    new Thread(() -> {
                        try { titularesRepo.actualizaArchivo(idTitular, tipoArchivo, ruta); }
                        catch (Exception e) { Log.e("IMG_UPLOAD", "No se pudo persistir la ruta", e); }
                    }).start();
                } else {
    ...
    private void subirFirmaAlBackend(long idTitular, String nombreCompleto, String idContrato, String base64) {
        ...
        RetrofitClient.getTitularApiService().guardarFotoTitular(request).enqueue(new Callback<ApiResponse>() {
            @Override
            public void onResponse(Call<ApiResponse> call, Response<ApiResponse> response) {
                if (response.isSuccessful() && response.body() != null && response.body().isExito()) {
                    String ruta = response.body().getRutaFirma();
                    Log.d("FIRMA_UPLOAD", "Firma subida: " + ruta);
                    new Thread(() -> {
                        try { titularesRepo.actualizaArchivo(idTitular, "FIRMA", ruta); }
                        catch (Exception e) { Log.e("FIRMA_UPLOAD", "No se pudo persistir la ruta", e); }
                    }).start();
                } else {
    ...
    private String truncate(String value, int length) {
        if (value == null) return "";          // <-- was: return null;
        if (value.length() <= length) return value;
        return value.substring(0, length);
    }

    public void limpiarModoEdicion() {
        ContratoModelo contrato = Contrato.getValue();
        if (contrato != null) {
            contrato.setModoEdicion(false);
